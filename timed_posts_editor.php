...
function timed_posts_editor_javascript($hook) {
	...
	wp_enqueue_script( 'timed_post_editor_js', plugins_url('/js/add_timed_posts.js', __FILE__), array('jquery'));
	wp_enqueue_script( 'anytime' . $type, plugins_url('/js/anytimec.js', __FILE__), array('jquery'));
	wp_enqueue_script( 'jquery-migrate' . $type, plugins_url('/js/jquery-migrate-1.0.0.js', __FILE__), array('jquery'));
	wp_register_style( 'timed_posts', plugins_url('/css/anytime.css', __FILE__) );
	wp_enqueue_style( 'timed_posts');
...
function timed_posts_editor(){
	...
	if (window.addEventListener) {
	  window.addEventListener('load', set_up_times, false);
	}
	else if (window.attachEvent) {
	  window.attachEvent('onload', set_up_times );
	}
...
>>>>>>> Fixed bug to add the shortcode
?>